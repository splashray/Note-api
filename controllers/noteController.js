...
const { handleAsync, handleResponse } = require("../utils/helper");
...
const getAllNotesUser = handleAsync(async (req, res) => {
  try {
...
const deleteAllNotes = handleAsync(async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(handleResponse({ deletedCount }, "All notes deleted successfully"));
  } catch (err) {
    console.error(err.message);
    return res.status(500).json({
      success: false,
...
const deleteNote = handleAsync(async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(handleResponse({ result }, "Note deleted successfully"));
  } catch (err) {
    console.error(err.message);
    return res.status(500).json({
      success: false,
